#!/bin/bash
#
# BC baseline for SoftDG-Token-Mask-Offline comparison.
#
# Usage:
#   sbatch run_bc.sh train          -- BC on all completions (BC-all baseline)
#   sbatch run_bc.sh train-correct  -- BC on correct completions only (BC-correct baseline)
...
CORRECT_ONLY="${CORRECT_ONLY:-0}"

# Output paths vary by BC mode
if [ "${CORRECT_ONLY}" = "1" ]; then
    CHECKPOINT_DIR="${CHECKPOINT_DIR:-${SCRATCH}/checkpoints/bc_tm_math_correct_only}"
    MERGED_DIR="${MERGED_DIR:-${SCRATCH}/merged/bc_tm_math_correct_only}"
    RUN_NAME="bc-badteacher-0.5B-correct-only"
else
    CHECKPOINT_DIR="${CHECKPOINT_DIR:-${SCRATCH}/checkpoints/bc_tm_math_all}"
    MERGED_DIR="${MERGED_DIR:-${SCRATCH}/merged/bc_tm_math_all}"
...
echo "=== BC baseline (SoftDG-TM comparison) ==="
echo "  Student:       ${STUDENT_MODEL}"
echo "  Rollout:       ${ROLLOUT_PATH}"
echo "  Correct only:  ${CORRECT_ONLY}"
...
# Build correct-only flag
CORRECT_ONLY_FLAG=""
if [ "${CORRECT_ONLY}" = "1" ]; then
    CORRECT_ONLY_FLAG="--filter_correct_only"
fi

CMD="${1:-train}"

if [ "$CMD" = "train" ] || [ "$CMD" = "train-correct" ]; then
    # train-correct is shorthand for CORRECT_ONLY=1 train
    if [ "$CMD" = "train-correct" ]; then
        CORRECT_ONLY_FLAG="--filter_correct_only"
        CHECKPOINT_DIR="${SCRATCH}/checkpoints/bc_tm_math_correct_only"
        MERGED_DIR="${SCRATCH}/merged/bc_tm_math_correct_only"
        RUN_NAME="bc-badteacher-0.5B-correct-only"
    fi
    ...
    echo "Usage: sbatch run_bc.sh {train|train-correct|eval [checkpoint|latest]|eval-baseline}"
    exit 1
fi
