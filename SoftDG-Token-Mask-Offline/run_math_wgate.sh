#!/bin/bash
#
# SoftDG-Token-Mask-Offline — DG-weighted selected-token PG (wgate variant).
#
# PG rule: PG_signal_t = training_signal_i * DG(token_t) for selected tokens.
# Identical to run_math.sh except:
#   - Passes --pg_weighting to train.py.
#   - RUN_TAG gets _wgate suffix.
#   - WANDB_PROJECT defaults to softdg-token-mask-math-wgate.
#
# Usage:
#   sbatch run_math_wgate.sh train
#   sbatch run_math_wgate.sh sanity
#   sbatch run_math_wgate.sh eval [checkpoint|latest]
#   sbatch run_math_wgate.sh eval-baseline
#
# Key env-var overrides (same as run_math.sh):
...
#SBATCH --job-name=softdg-tm-wgate
...
# eta tag: replace '.' with 'p'
ETA_TAG=$(printf "%s" "${DG_ETA}" | tr '.' 'p')

# Run tag includes _wgate suffix to distinguish from unweighted runs
RUN_TAG="${REWARD_CODING}_${TRAINING_SIGNAL}_${LOSS_TYPE}_eta${ETA_TAG}_thr${SOFTDG_GATE_THRESHOLD}_wgate"
...
WANDB_PROJECT="${WANDB_PROJECT:-softdg-token-mask-math-wgate}"
...
echo "=== SoftDG-Token-Mask-Offline (DG-weighted PG) on MATH ==="
...
save_result() {
    ...
    echo "Config: reward_coding=${REWARD_CODING} training_signal=${TRAINING_SIGNAL} threshold=${SOFTDG_GATE_THRESHOLD} eta=${DG_ETA} pg_weighting=true" >> "${result_file}"
...
if [ "$CMD" = "train" ]; then
    ...
    echo "=== Sanity check: wgate, 32 effective tokens ==="
    CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch \
        --config_file configs/accelerate_ddp_4gpu.yaml \
        train.py \
        --target_model "${STUDENT_MODEL}" \
    ...
    echo "Check logs for: PG mask nontrivial, softdg/mean_selected_gate logged, softdg/pg_mass_weighted logged, KL active, token counter increments"
    ...
    echo "Usage: sbatch run_math_wgate.sh {train|sanity|eval [checkpoint|latest]|eval-baseline}"
    exit 1
fi
