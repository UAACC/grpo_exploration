#!/bin/bash
#
# Method B: Weighted Mixture GRPO (GSM8K)
...
#
# Usage:
#   sbatch --job-name=mixture-B-rollouts run_method_B.sh rollouts
#   sbatch --job-name=mixture-B run_method_B.sh train
#   sbatch --job-name=mixture-B-lam01 run_method_B.sh train-lambda 0.1
...
#SBATCH --mem=256G
#SBATCH --output=logs/mixture_B/%x-%j.out
#SBATCH --error=logs/mixture_B/%x-%j.err
...
OUTPUT_DIR="${SCRATCH}/checkpoints/mixture_B_gsm8k"
MERGED_DIR="${SCRATCH}/merged/mixture_B_gsm8k_merged"
...
export WANDB_PROJECT="mixture-grpo-B"
cd "${WORK_DIR}"

mkdir -p logs/mixture_B
...
train)
    LAMBDA="${2:-0.3}"
    echo "=== Method B: Weighted Mixture GRPO (GSM8K, lambda=${LAMBDA}) ==="
    accelerate launch \
...
train-lambda)
    LAMBDA="${2:-0.3}"
    echo "=== Method B: lambda=${LAMBDA} ==="
    OUTPUT_LAMBDA="${SCRATCH}/checkpoints/mixture_B_lambda${LAMBDA}"
    accelerate launch \
        --config_file "${WORK_DIR}/../offline_grpo/configs/accelerate_ddp_4gpu.yaml" \
        method_B_weighted/train.py \
        --teacher_rollout_path "${ROLLOUT_PATH}" \
...
*)
    echo "Usage: sbatch --job-name=<name> run_method_B.sh {rollouts|train|train-lambda <val>|eval|eval-baseline}"
    exit 1
    ;;
