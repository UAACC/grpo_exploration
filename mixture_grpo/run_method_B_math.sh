#!/bin/bash
#
# Method B: Weighted Mixture GRPO (MATH)
...
#
# Usage:
#   sbatch --job-name=mixture-B-math run_method_B_math.sh train
#   sbatch --job-name=mixture-B-math-eval run_method_B_math.sh eval
#
...
OUTPUT_DIR="${SCRATCH}/checkpoints/mixture_B_math"
MERGED_DIR="${SCRATCH}/merged/mixture_B_math_merged"
...
train)
    LAMBDA="${2:-0.3}"
    echo "=== Method B: Weighted Mixture GRPO (MATH, 1 epoch, lambda=${LAMBDA}) ==="
    accelerate launch \
...
eval)
    ...
    echo "=== Evaluating Method B MATH (${CKPT}) ==="
    python evaluate.py \
...
*)
    echo "Usage: sbatch --job-name=<name> run_method_B_math.sh {train|eval [checkpoint]|eval-baseline}"
    exit 1
    ;;
