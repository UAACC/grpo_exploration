...
#
# Hyperparameters mirror DG-offline R1 runs (the comparable baseline) except
# for DG-specific gate parameters: per_device_batch=1, grad_accum=8,
# num_generations=4, max_completion_length=8192. The 8K completion cap
# right-truncates ~10% of R1's longest reasoning chains during the IS-ratio
# forward pass — acceptable for OG since the loss is advantage-weighted
# logprob ratio (truncation produces a noisier signal, not a wrong target,
# unlike BC where truncation deletes the \boxed{} answer).
#
# Usage:
...
echo "    epochs=1, per_device_batch=1, grad_accum=8, num_gen=4"
